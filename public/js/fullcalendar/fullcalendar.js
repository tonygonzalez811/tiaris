	...
	bgEventSegStyles: function(seg) {
		...
		if (backgroundColor) {
            if (backgroundColor == '#fff') {
                return 'border:1px solid #5e87b0;background-color:' + backgroundColor;
            }
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
        if (window.creating_new_event) return;
		var view = this.view;
		var event = seg.event;
        ...
        classes.push('group' + group_id);
        classes.push('event' + id);
		if (has_comment) {
			classes.push('tip');
		}
		if (!is_cita) {
        ...
        if (event.has_many == 1) {
            classes.push('has-many');
        }

		// Only display a timed events time if it is the starting segment
		if (!event.allDay && seg.isStart) {
			timeHtml = '<span class="fc-time">' + htmlEscape(this.getEventTimeText(event)) + (typeof doctor_name != 'undefined' ? (' <span class="badge" style="background-color:' + doctor_color + '">' + doctor_name + '</span>'):'') + comment_icon + atention + '</span>';
		}
...
var TimeGrid = Grid.extend({
	...
	slatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			html +=
				'<tr ' + (!minutes ? '' : ('class="fc-minor' + (minutes == 30 ? ' fc-half' : '') + '"')) + '>' +
					(!isRTL ? axisHtml : '') +
	...
	function renderSection(position) {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
                ...
                //added by alfredo
                if (position == 'center') {
                    sectionEl.append( $('<span id="total_citas"></span>') );
                }
